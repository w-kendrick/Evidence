...
{
	if (ProneCharacterOwner)
	{
		return ProneCharacterOwner->GetMoveSpeed();
	}
	return 0.0f;
}
...
bool UProneMovement::IsMovingForward() const
{
	if (!PawnOwner)
	{
		return false;
	...
	FNetworkPredictionData_Client_Character& ClientData)
{
	...
	bWantsToSprint = Cast<AProneCharacter>(C)->GetProneCharacterMovement()->bWantsToSprint;
}
...
void UProneMovement::UpdateFromCompressedFlags(uint8 Flags)
{
	...
	bWantsToProne = ((Flags & FSavedMove_Character_Prone::FLAG_Prone) != 0);
	bWantsToSprint = ((Flags & FSavedMove_Character_Prone::FLAG_Sprint) != 0);
}
